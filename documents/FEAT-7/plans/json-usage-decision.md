...
The source of truth for *values* shifts from the i18n JSON files to the `site_content` table. The JSON files stay in the repo, but their role is reduced: they are now a **key-schema manifest** and a **first-time seed**, not a live content store.
