...
export default function DashboardIndex() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="font-serif text-2xl font-bold text-text-primary">{t('dashboard.title')}</h1>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        {kpi.map((k) => (
          <Card key={k.label} className="shadow-sm transition-shadow hover:shadow-md">
            ...
            <CardContent>
              <p className="font-serif text-3xl font-bold text-primary-700">{k.value}</p>
            </CardContent>
          </Card>
        ))}
      </div>
                ...
                return (
                  <div key={p.product_id} className="space-y-1">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-text-secondary">{p.total_quantity}</span>
                    </div>
      ...
      <Card>
        ...
        <CardContent className="flex flex-wrap gap-3">
          {Object.entries(data.ordersByStatus).map(([status, count]) => (
            <div
              key={status}
              className="min-w-[120px] rounded-md border border-border-light bg-bg-elevated px-4 py-3"
            >
